...
def get_paths_by_dist_function():
    address = request.args.get('address')
    run_distance = request.args.get('run_distance')
    user_id = request.args.get('user_id')
    is_closed = request.args.get('loopback')
    n_routes = request.args.get('n_routes')

    routes = get_paths_by_dist(address, run_distance, user_id, is_closed = is_closed, n_routes = n_routes)
    if routes:
