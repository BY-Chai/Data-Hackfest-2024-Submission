...
import pandas as pd
import json
# GeoJSoN uses lon-lat order for storing coordinates
...
def get_paths_by_dist(start_address: str, run_distance: float, user_id, is_closed = False, n_routes = 10):
    # FOR TESTING ONLY
    #dist_dataframe = pd.read_csv("CNtowerstart_2000m", encoding='utf-8')
    ...
    out_file = open("routes.json", "w", encoding="utf-8") 
    json.dump(all_routes, out_file, indent=6)
...
if __name__ == "__main__":
    ...
    #start_coords = get_coords_for_address(start_address, user_id)
    #dist_dataframe = find_potential_distances(start_coords, 2000, user_id)
    get_paths_by_dist("CN Tower", 1500, user_id)
    
"""
def get_paths_by_endpoint(start_address: str, end_address: str):
    get_directions(start_coords, end_coords)
"""